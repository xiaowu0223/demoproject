...
#### 消费者组
多个消费者划分到一个组，监听同一个队列
* 消息分流：队列当中的消息分流给组内不同消费者，加快消息处理速度
* 消息标识：消费者组维护一个标识，记录最后一个被处理的消息，即使消费者组宕机重启，也会从标识后读取消息，确保每个消息都被消费
* 消息确认：消费者获取消息后，消息处于pending状态，并存入pending-list，处理完成后需要通过XACK确认消息，标记消息为已处理，从pending-list当中移除
基本命令
![img_129.png](img_129.png)
![img_130.png](img_130.png)
![img_131.png](img_131.png)
![img_132.png](img_132.png)
#### 基于redis的Stream结构实现异步秒杀下单
